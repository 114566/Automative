...
// component
import LightBox from '../../../../components/Lightox';

// types
import { ImageType } from '../../../../components/Lightox/lightBox';
...
const Gallery = ({ galleryItems }: GalleryProps) => {
    ...
    const [category, setCategory] = useState<string>('Übersicht');
    ...
    const filterImages = (category: string) => {
        ...
        setTimeout(() => {
            const galleryAlbums =
                category === 'Übersicht' ? galleryItems : galleryItems.filter((album) => album.category?.includes(category));
                    ...
                    <div className="text-center filter-menu">
                        <Link
                            ...
                            className={classNames('filter-menu-item', 'me-1', {
                                active: category === 'Übersicht',
                            })}
                            onClick={() => filterImages('Übersicht')}
                        >
                            Übersicht
                        </Link>
                        <Link
                            to="#"
                            className={classNames('filter-menu-item', 'me-1', {
                                active: category === 'Finanzen',
                            })}
                            onClick={() => filterImages('Finanzen')}
                        >
                            Finanzen
                        </Link>
                        <Link
